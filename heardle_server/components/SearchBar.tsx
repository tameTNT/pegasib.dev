...
export default function SearchBar(
  ...
  useEffect(() => { // Fetch all songs when the selected artist name changes
    ...
    fetchSongs()
      .then(() => {
        console.debug(
          `${props.artistVariant.value.name} songs fetched successfully.`,
        );
      }).catch((error) => {
  ...
  return (
    <div class="relative text-sm w-full">
      {showSuggestions && suggestions.length > 0 && (
        ...
        >
          {suggestions.map((song) => ( // todo: interacts weirdly with keyboards on mobile (can't be scrolled until keyboard dismissed)
            <div
              key={song}
              tabindex={0}
